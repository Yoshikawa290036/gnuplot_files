...
PM3D=sprintf("data/4_0REC/uvwpm3d%07d.csv2",tstep)
...
recnst="data/4_0REC_V2/m2andm3_0070000.csv"
residu="data/4_0REC_V2/m2andm3_0070000.csv"

m23=sprintf("data/4_0REC_V2/m2andm3_%07d.csv", tstep)
m45=sprintf("data/4_0REC/m4andm5_%07d.csv", tstep)
...
set label 31 at graph 0.02,1.12 '(b)~mode2$+$mode3' front
...
splot \
PM3D using ($1):($2 - inner):($6*$2) with pm3d title '' ,\
m23 every ev:ev:3 using ($1/64):(($2-0.111111)/64):($1-$1):($4*adj):($5*adj):($1-$1):(sqrt($4*$4+$5*$5)) with vectors linecolor "black" linewidth 3 title ""


#################################################################################


set label 31 at graph 0.02,1.12 '(c)~mode4$+$mode5' front
...
splot \
PM3D using ($1):($2 - inner):($6*$2) with pm3d title '' ,\
m45 every ev:ev:3 using ($1/64):(($2-0.111111)/64):($1-$1):($4*adj):($5*adj):($1-$1):(sqrt($4*$4+$5*$5)) with vectors linecolor "black" linewidth 3 title ""
